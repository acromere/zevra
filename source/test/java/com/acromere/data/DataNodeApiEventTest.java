...
class DataNodeApiEventTest extends BaseDataNodeTest {
	...
	void testGetAndSetValueEvents() {
		List<DataNodeEvent> events = new ArrayList<>();
		...
		NodeEventAssert.assertThat( events.get( 0 ) ).hasEventState( data, DataNodeEvent.VALUE_CHANGED, "x", null, 1.0 );
		NodeEventAssert.assertThat( data.event( index++ ) ).hasEventState( TxnEvent.COMMIT_BEGIN );
		NodeEventAssert.assertThat( data.event( index++ ) ).hasEventState( DataNodeEvent.VALUE_CHANGED, "x", null, 1.0 );
		...
		assertThat( data.<Object> getValue( "z" ) ).isNull();
		NodeAssert.assertThat( data ).hasStates( true, false, 1, 0 );
		NodeEventAssert.assertThat( events.get( 1 ) ).hasEventState( data, DataNodeEvent.VALUE_CHANGED, "x", 1.0, 0.0 );
		NodeEventAssert.assertThat( data.event( index++ ) ).hasEventState( TxnEvent.COMMIT_BEGIN );
		NodeEventAssert.assertThat( data.event( index++ ) ).hasEventState( DataNodeEvent.VALUE_CHANGED, "x", 1.0, 0.0 );
		NodeEventAssert.assertThat( data.event( index++ ) ).hasEventState( DataNodeEvent.NODE_CHANGED );
		NodeEventAssert.assertThat( data.event( index++ ) ).hasEventState( TxnEvent.COMMIT_SUCCESS );
		NodeEventAssert.assertThat( data.event( index++ ) ).hasEventState( TxnEvent.COMMIT_END );
		assertThat( data.getEventCount() ).isEqualTo( index );
	...
	void testNodeSetAddRemoveEvents() {
		...
		NodeEventAssert.assertThat( item.event( itemIndex++ ) ).hasEventState( DataNodeEvent.ADDED );
		...
		NodeEventAssert.assertThat( data.event( index++ ) ).hasEventState( item.getTrueParent(), DataNodeEvent.CHILD_ADDED, "items", item.getCollectionId(), null, item );
		NodeEventAssert.assertThat( data.event( index++ ) ).hasEventState( item.getTrueParent(), DataNodeEvent.VALUE_CHANGED, "items", item.getCollectionId(), null, item );
		NodeEventAssert.assertThat( data.event( index++ ) ).hasEventState( DataNodeEvent.MODIFIED );
		NodeEventAssert.assertThat( data.event( index++ ) ).hasEventState( DataNodeEvent.NODE_CHANGED );
		...
		NodeEventAssert.assertThat( item.event( itemIndex++ ) ).hasEventState( DataNodeEvent.REMOVED );
		assertThat( item.getEventCount() ).isEqualTo( itemIndex );
		NodeEventAssert.assertThat( data.event( index++ ) ).hasEventState( items, TxnEvent.COMMIT_BEGIN );
		NodeEventAssert.assertThat( data.event( index++ ) ).hasEventState( set, DataNodeEvent.CHILD_REMOVED, "items", item.getCollectionId(), item, null );
		NodeEventAssert.assertThat( data.event( index++ ) ).hasEventState( set, DataNodeEvent.VALUE_CHANGED, "items", item.getCollectionId(), item, null );
	...
	void testResourceChangesOnChildCausesParentNodeChangedEvent() {
		...
		NodeEventAssert.assertThat( parent, parentIndex++ ).hasEventState( child, DataNodeEvent.VALUE_CHANGED, "name", null, "mock" );
		NodeEventAssert.assertThat( parent, parentIndex++ ).hasEventState( DataNodeEvent.NODE_CHANGED );
		...
		NodeEventAssert.assertThat( child, childIndex++ ).hasEventState( DataNodeEvent.VALUE_CHANGED, "name", null, "mock" );
		NodeEventAssert.assertThat( child, childIndex++ ).hasEventState( DataNodeEvent.NODE_CHANGED );
		...
		NodeAssert.assertThat( child ).hasStates( false, false, 0, 0 );
		NodeEventAssert.assertThat( parent, parentIndex++ ).hasEventState( child, TxnEvent.COMMIT_BEGIN );
		NodeEventAssert.assertThat( parent, parentIndex++ ).hasEventState( child, DataNodeEvent.VALUE_CHANGED, "name", "mock", null );
		NodeEventAssert.assertThat( parent, parentIndex++ ).hasEventState( DataNodeEvent.NODE_CHANGED );
		NodeEventAssert.assertThat( parent, parentIndex++ ).hasEventState( child, TxnEvent.COMMIT_SUCCESS );
		NodeEventAssert.assertThat( parent, parentIndex++ ).hasEventState( child, TxnEvent.COMMIT_END );
		NodeEventAssert.assertThat( child, childIndex++ ).hasEventState( TxnEvent.COMMIT_BEGIN );
		NodeEventAssert.assertThat( child, childIndex++ ).hasEventState( DataNodeEvent.VALUE_CHANGED, "name", "mock", null );
		NodeEventAssert.assertThat( child, childIndex++ ).hasEventState( DataNodeEvent.NODE_CHANGED );
		NodeEventAssert.assertThat( child, childIndex++ ).hasEventState( TxnEvent.COMMIT_SUCCESS );
		NodeEventAssert.assertThat( child, childIndex++ ).hasEventState( TxnEvent.COMMIT_END );
		assertThat( parent.getEventCount() ).isEqualTo( parentIndex );
	...
	void testDataEventNotification() {
		...
		NodeAssert.assertThat( data ).hasStates( true, false, 1, 0 );
		NodeEventAssert.assertThat( data.event( index++ ) ).hasEventState( TxnEvent.COMMIT_BEGIN );
		NodeEventAssert.assertThat( data.event( index++ ) ).hasEventState( DataNodeEvent.VALUE_CHANGED, "key", null, "value0" );
		NodeEventAssert.assertThat( data.event( index++ ) ).hasEventState( DataNodeEvent.MODIFIED );
		NodeEventAssert.assertThat( data.event( index++ ) ).hasEventState( DataNodeEvent.NODE_CHANGED );
		NodeEventAssert.assertThat( data.event( index++ ) ).hasEventState( TxnEvent.COMMIT_SUCCESS );
		NodeEventAssert.assertThat( data.event( index++ ) ).hasEventState( TxnEvent.COMMIT_END );
		...
		NodeAssert.assertThat( data ).hasStates( true, false, 1, 0 );
		NodeEventAssert.assertThat( data.event( index++ ) ).hasEventState( TxnEvent.COMMIT_BEGIN );
		NodeEventAssert.assertThat( data.event( index++ ) ).hasEventState( DataNodeEvent.VALUE_CHANGED, "key", "value0", "value1" );
		NodeEventAssert.assertThat( data.event( index++ ) ).hasEventState( DataNodeEvent.NODE_CHANGED );
		NodeEventAssert.assertThat( data.event( index++ ) ).hasEventState( TxnEvent.COMMIT_SUCCESS );
		NodeEventAssert.assertThat( data.event( index++ ) ).hasEventState( TxnEvent.COMMIT_END );
		assertThat( data.getEventCount() ).isEqualTo( index );
		...
		NodeAssert.assertThat( data ).hasStates( false, false, 0, 0 );
		NodeEventAssert.assertThat( data.event( index++ ) ).hasEventState( TxnEvent.COMMIT_BEGIN );
		NodeEventAssert.assertThat( data.event( index++ ) ).hasEventState( DataNodeEvent.VALUE_CHANGED, "key", "value1", null );
		NodeEventAssert.assertThat( data.event( index++ ) ).hasEventState( DataNodeEvent.UNMODIFIED );
		NodeEventAssert.assertThat( data.event( index++ ) ).hasEventState( DataNodeEvent.NODE_CHANGED );
		NodeEventAssert.assertThat( data.event( index++ ) ).hasEventState( TxnEvent.COMMIT_SUCCESS );
		NodeEventAssert.assertThat( data.event( index++ ) ).hasEventState( TxnEvent.COMMIT_END );
	...
	void testCollapsingEventsWithTransaction() throws Exception {
		...
		NodeAssert.assertThat( data ).hasStates( true, false, 1, 0 );
		NodeEventAssert.assertThat( data.event( index++ ) ).hasEventState( TxnEvent.COMMIT_BEGIN );
		NodeEventAssert.assertThat( data.event( index++ ) ).hasEventState( DataNodeEvent.VALUE_CHANGED, "a", null, "5" );
		NodeEventAssert.assertThat( data.event( index++ ) ).hasEventState( DataNodeEvent.MODIFIED );
		NodeEventAssert.assertThat( data.event( index++ ) ).hasEventState( DataNodeEvent.NODE_CHANGED );
		NodeEventAssert.assertThat( data.event( index++ ) ).hasEventState( TxnEvent.COMMIT_SUCCESS );
		NodeEventAssert.assertThat( data.event( index++ ) ).hasEventState( TxnEvent.COMMIT_END );
